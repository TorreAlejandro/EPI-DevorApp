...
function Fail  { Write-Host "`n  ✖ $args`n" -ForegroundColor Red; exit 1 }
function Need  {
...
if ($RunBackend) {
    ...
    if (-not (Get-Command poetry -ErrorAction SilentlyContinue)) {
        ...
        if (Test-Path $fallback) { $env:PATH = "$env:APPDATA\Python\Scripts;$env:PATH" }
        else { Fail "Poetry not found. Install: pip install poetry" }
    }
    Ok "$(poetry --version)"
}
...
if ($Mode -eq "docker") {
    ...
    switch ($Component) {
        "backend"  { $composeArgs += @("db", "backend") }
